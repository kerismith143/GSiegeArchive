...
int ShowTab(HWND hWnd)
{
	...
	if ( hWnd == GlobalSettings.hSiegeTab[0] ) // Flame Ram tab
		FlameRamToggleElements();
	else if ( hWnd == GlobalSettings.hSiegeTab[1] ) // Catapult tab
		CatapultToggleElements();
	else if ( hWnd == GlobalSettings.hSiegeTab[2] ) // Trebuchet tab
...
HWND TabButtonBuilder(int nButtonIndex, int nMaxButtons, char *szText)
{
	...
	nButtonX = abs(UserSettings.ClientWidth / nMaxButtons) * nButtonIndex;
	nButtonY = 0;
	nButtonWidth = abs(UserSettings.ClientWidth / nMaxButtons);
	...
	SendMessage(hWnd, WM_SETFONT, (WPARAM)GlobalSettings.hFont, 0);
...
void TabWindowClassBuilder()
{
	...
	// Build tab buttons.
	for ( n = 0; n < MAXTABS; n++ )
		GlobalSettings.hSiegeTab[n] = TabButtonBuilder(n, MAXTABS, lpszTabNames[n]);
}